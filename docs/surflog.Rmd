...
As an example, I used an example dataset with 86 records of surf sessions over four years in Monterey, California to represent a central California surfer. Below, I plot the monthly number of surf sessions:

![Figure 1](../figs/plot_dat_yrm_n.png)

But the real benefit of the app will be to inform the user whether a particular site, given the current conditions, will be worth surfing. The statistical model will take the current NOAA, tide, and weather conditions, and integrate them with the historical user data to predict a metric of quality (e.g., number of predicted rides, rating). 

For example, using my central California surfer I can make predictions using a simple linear model with swell period, swell height, swell direction, and tide. The predicted number of rides for each variable is presented below:
...
This analysis is based on an example surf log, together with hand-entered data for the swell and tide conditions. However, the data scraping can be automated. Moreover, the statistical model can likely be improved. I foresee the following steps for completing the app over the next three months:
